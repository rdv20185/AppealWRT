...
leave_d between  '01.01.2018'
and '31.12.2018'
and d.id_demand  = hc.demand_id
and d.period between 201801 and 201901
...
and dat_end between  '01.01.2018' and '31.12.2018'
and d.id_demand  = demand_id and d.period between 201801 and 201901
...
leave_d between  '01.01.2018' and '31.12.2018'
and ds_final between 'O02' and 'O07.99'--  and not  trim(ds_final) in (select mkb from abort_mkb_mz)
and substr(ds_final,1,3)  <> 'O03'
and hc.id not in (select id_pred from otkl_id)
and pi.id = hc.id
...
and d.period between 201801 and 201901
 and 
 (
  -------- экстренность добавлена Геленой 15.02.2018
hc.hosp_extr  = 1
------ 
or 
-------- O02
...
and 
 -- changed 20.03.2018 hc.lpuid  in (1,180,100,110,187,134,103,123,607,610,618,628,634,186,140,647)
...
and substr(mkb,1,3)  <> 'O03'
and p.id not in (select id_pred from otkl_id)
and dat_end  between  '01.01.2018' and '31.12.2018' group by substr(mkb,1,3) )
...
leave_d between  '01.01.2018' and '31.12.2018' and d.id_demand  = hc.demand_id and d.period between 201801 and 201801
and ds_final between 'O02' and 'O07.99' -- and not  trim(ds_final) in (select mkb from abort_mkb_mz)
and substr(ds_final,1,3)  <> 'O03'
...
and mkb between  'O02' and 'O07.99'--and not  trim(mkb) in (select mkb from abort_mkb_mz) 
and dat_end between  '01.01.2018' and '31.12.2018'  and d.id_demand  = demand_id and d.period between 201801 and 201801
and substr(mkb,1,3)  <> 'O03'
...
and substr(ds_final,1,3)  <> 'O03'
and hc.id not in (select id_pred from otkl_id@link_collect2018)
 and pi.id = hc.id  and d.id_demand  = hc.demand_id and d.period between 201801 and 201801
 and 
 (
  -------- экстренность добавлена Геленой 15.02.2018
hc.hosp_extr  = 1
...
and mkb between 'O02' and 'O07.99' -- and not trim(mkb) in (select mkb from abort_mkb_mz)
 and d.id_demand  = demand_id and d.period between 201801 and 201801
 and substr(mkb,1,3)  <> 'O03'
 and p.id not in (select id_pred from otkl_id@link_collect2018)
and dat_end  between  '01.01.2018' and '31.12.2018' group by substr(mkb,1,3) )
-----------------
